...
class RoleChoiceView(discord.ui.View):
    ...
    async def dps_choice(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item in teams_data[self.team_key]["dps"]:
            if item["user"].id == interaction.user.id:
    ...
    async def support_choice(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item in teams_data[self.team_key]["support"]:
            if item["user"].id == interaction.user.id:
                await interaction.response.send_message("❌ لقد قمت بالتسجيل في رول الهيلر مسبقاً!", ephemeral=True)
                return
